...
def preprocess_hotel_data(hotel_df):
    ...
    # Create user behavior features - only use columns that exist
    agg_dict = {}
    
    if 'stay_duration' in df.columns:
        agg_dict['stay_duration'] = ['mean', 'std', 'count']
    
    if has_booking_lead_time:
        agg_dict['booking_lead_time'] = ['mean', 'std']
    
    if 'city_id' in df.columns:
        agg_dict['city_id'] = 'nunique'
    
    if 'hotel_country' in df.columns:
        agg_dict['hotel_country'] = 'nunique'
        ...
        for col in user_stats.columns:
            if isinstance(col, tuple):
                if col[1] == 'mean':
                    ...
                    new_columns.append(f'std_{col[0]}')
                elif col[1] == 'count':
                    new_columns.append(f'{col[0]}_frequency')
                elif col[1] == 'nunique':
    ...
    feature_list = []
    if 'stay_duration' in df.columns:
        feature_list.append('stay_duration')
    if has_booking_lead_time:
        feature_list.append('booking_lead_time')
    feature_list.extend(['temporal features', 'user behavior'])
    print(f"✓ Added features: {', '.join(feature_list)}")
...
def preprocess_trips_data(trips_df):
    ...
    if 'Accommodation cost' in df.columns and 'Duration (days)' in df.columns:
        ...
        # print("Accommodation cost data types and sample values:")
        # print(pd.to_numeric(df['Accommodation cost']).dtype)
        # print(pd.to_numeric(df['Accommodation cost']).head())
        # print("\nDuration data types and sample values:")
        # print(pd.to_numeric(df['Duration (days)'].dtype))
        # print(pd.to_numeric(df['Duration (days)'].head()))

        # df['accommodation_cost_per_day'] = df['Accommodation cost'] / df['Duration (days)']
        # df['accommodation_cost_per_day'] = pd.to_numeric(df['Accommodation cost']) / df['Duration (days)']

        # Clean the accommodation cost column: remove $, whitespace, commas, then convert to numeric
        df['Accommodation cost'] = pd.to_numeric(
            df['Accommodation cost'].astype(str).str.replace(r'[$,\s]', '', regex=True),
            errors='coerce'
        )
    ...
    if 'Transportation cost' in df.columns and 'Duration (days)' in df.columns:
        df['Transportation cost']=pd.to_numeric(
            df['Transportation cost'].astype(str).str.replace(r'[$,\s]', '', regex=True),
            errors='coerce'
        )
    ...
    if 'Traveler name' in df.columns:
        ...
        if 'Duration (days)' in df.columns:
            agg_dict['Duration (days)'] = ['mean', 'std', 'count']
        
        if 'total_cost' in df.columns:
            agg_dict['total_cost'] = ['mean', 'std']
        
        if 'Destination' in df.columns:
            agg_dict['Destination'] = 'nunique'
        
        if agg_dict:  # Only perform aggregation if we have columns to aggregate
            traveler_stats = df.groupby('Traveler name').agg(agg_dict).round(2)
            
            # Flatten column names
            new_columns = []
            for col in traveler_stats.columns:
                if isinstance(col, tuple):
                    if col[1] == 'mean':
                        new_columns.append(f'avg_trip_{col[0].lower().replace(" ", "_").replace("(", "").replace(")", "")}')
                    elif col[1] == 'std':
                        new_columns.append(f'std_trip_{col[0].lower().replace(" ", "_").replace("(", "").replace(")", "")}')
                    elif col[1] == 'count':
                        new_columns.append('trip_frequency')
                    elif col[1] == 'nunique':
                        new_columns.append('destinations_visited')
                    else:
                        new_columns.append(f'{col[0]}_{col[1]}')
                else:
                    new_columns.append(str(col))
            
            traveler_stats.columns = new_columns
            
            # Handle NaN values
            for col in traveler_stats.columns:
                if col.startswith('std_'):
                    traveler_stats[col] = traveler_stats[col].fillna(0)
...
def train_models(X, y, preprocessor, model_name):
    ...
    print(f"Class distribution for {model_name}:")
    print(class_counts)
    
    # Option 1: Remove classes with only 1 example
    classes_to_keep = class_counts[class_counts >= 2].index
    mask = y.isin(classes_to_keep)
    X_filtered = X[mask]
    y_filtered = y[mask]
    
    print(f"Removed {len(y) - len(y_filtered)} examples with rare classes")
    
    # Now use stratified split with filtered data
